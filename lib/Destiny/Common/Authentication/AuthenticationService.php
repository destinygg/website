...
class AuthenticationService extends Service {
    ...
    public function validateUsername(string $username) {
        ...
        foreach ($this->getEmotesForValidation() as $e) {
            $prefix = $e['prefix'];
            $normalizedPrefix = strtolower($e['prefix']);
            ...
            if (strcmp($normalizedUsername, $normalizedPrefix) === 0) {
                throw new Exception("Username is invalid because it matches the emote $prefix.");
            }

            // Ensure the beginning of the username doesn't contain the entire
            // emote prefix. `Jamstinycakes` fails if `JAMSTINY` exists.
            if (strpos($normalizedUsername, $normalizedPrefix) === 0) {
                throw new Exception("Username is invalid because it starts with the emote $prefix.");
            }
            ...
            // If the first two letters of the username and the emote prefix
            // match, ensure the Levenshtein distance between the first `n`
            // letters of the username (where `n` is the length of the emote
            // prefix) and the emote prefix is <= 2. `Japstiny` fails because
            // the distance between `japstiny` and `jamstiny` is 1. Only one
            // letter has to be replaced/inserted/deleted to change `japstiny`
            // to `jamstiny`.
            ...
            if (substr($normalizedUsername, 0, 2) == substr($normalizedPrefix, 0, 2) && levenshtein($normalizedPrefix, $usernamePrefix) <= 2) {
                throw new Exception("Username is invalid because it has too many like characters to the emote $prefix.");
            }
        }
    }
