...
class SubscriptionsService extends Service {
	...
	public function createSubscriptionFromOrder(array $order, array $subscription, array $paymentProfile = null) {
		...
		$subscriptionStatus = ($order ['state'] == OrderStatus::COMPLETED) ? SubscriptionStatus::ACTIVE : SubscriptionStatus::PENDING;
		$subscriptionId = $this->addSubscription ( $order ['userId'], $start->format ( 'Y-m-d H:i:s' ), $end->format ( 'Y-m-d H:i:s' ), $subscriptionStatus, (! empty ( $paymentProfile )), Config::$a ['subscriptionType'], $subscription ['id'], $subscription ['tier'] );
		if (! empty ( $paymentProfile )) {
			$this->updateSubscriptionPaymentProfile ( $subscriptionId, $paymentProfile ['profileId'], true );
		}
	}
}