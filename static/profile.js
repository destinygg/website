...
var Feature = function () {
    function Feature(id, name, label) {
...
var mapping = new Map([['protected', features.PROTECTED], ['subscriber', features.SUBSCRIBER], ['flair9', features.SUBSCRIBERT0], ['flair1', features.SUBSCRIBERT2], ['flair13', features.SUBSCRIBERT1], ['flair3', features.SUBSCRIBERT3], ['flair8', features.SUBSCRIBERT4], ['vip', features.VIP], ['moderator', features.MODERATOR], ['admin', features.ADMIN], ['flair12', features.BROADCASTER], ['bot', features.BOT], ['flair11', features.BOT2], ['flair2', features.NOTABLE], ['flair4', features.TRUSTED], ['flair5', features.CONTRIBUTOR], ['flair6', features.COMPCHALLENGE], ['flair7', features.EVE], ['flair10', features.SC2], ['flair15', features.DGGBDAY]]);
...
exports.UrlFormatter = exports.MentionedUserFormatter = exports.HtmlTextFormatter = exports.GreenTextFormatter = exports.EmoteFormatter = undefined;
...
/** @var Array tlds */
var tlds = __webpack_require__(490);
var gtld = "(?:" + tlds.join('|') + ")";
var el = document.createElement('div');
var gemoteregex = void 0,
    twitchemoteregex = void 0;

var HtmlTextFormatter = function () {
    ...
    _createClass(HtmlTextFormatter, [{
        key: 'format',
        value: function format(chat, str) {
...
var EmoteFormatter = function () {
    function EmoteFormatter() {
    ...
    _createClass(EmoteFormatter, [{
        ...
        value: function format(chat, str) {
            ...
            var regex = message && message.user && message.user.hasFeature(_features2.default.SUBSCRIBERT0) || !message || !message.user ? twitchemoteregex : gemoteregex;
...
var GreenTextFormatter = function () {
    ...
    _createClass(GreenTextFormatter, [{
        ...
        value: function format(chat, str) {
            ...
            if (message.user && str.indexOf('&gt;') === 0) {
                if (message.user.hasAnyFeatures(_features2.default.SUBSCRIBER, _features2.default.SUBSCRIBERT0, _features2.default.SUBSCRIBERT1, _features2.default.SUBSCRIBERT2, _features2.default.SUBSCRIBERT3, _features2.default.SUBSCRIBERT4, _features2.default.NOTABLE, _features2.default.TRUSTED, _features2.default.CONTRIBUTOR, _features2.default.COMPCHALLENGE, _features2.default.ADMIN, _features2.default.MODERATOR)) str = '<span class="greentext">' + str + '</span>';'';
            }
            return str;
        }
    }]);
...
var MentionedUserFormatter = function () {
    function MentionedUserFormatter() {
    ...
    _createClass(UrlFormatter, [{
        ...
        value: function format(chat, str) {
            ...
            return str.replace(self.linkregex, function (url, scheme) {
                ...
                var m = decodedUrl.match(self.linkregex);
                if (m) {
...
exports.EmoteFormatter = EmoteFormatter;
exports.GreenTextFormatter = GreenTextFormatter;
exports.HtmlTextFormatter = HtmlTextFormatter;
exports.MentionedUserFormatter = MentionedUserFormatter;
...
var chatformatters = __webpack_require__(489);
var chatemotes = __webpack_require__(491);
var ctx = { emoticons: chatemotes.destiny, twitchemotes: chatemotes.twitch };
var formatters = [new chatformatters.HtmlTextFormatter(), new chatformatters.UrlFormatter(), new chatformatters.EmoteFormatter()];
...
$(function () {
    ...
    $('#compose.message-composition').each(function () {
        ...
        var saveStateOnClose = false;

        var disableMessageForm = function disableMessageForm() {
            $form.find('button,input,textarea').attr('disabled', 'disabled');
        };

        var enableMessageForm = function enableMessageForm() {
        ...
        var sendMessage = function sendMessage() {
            ...
            $.ajax({ type: 'post', url: '/profile/messages/send', data: { 'recipients': recipients, 'message': message } }).always(function () {
                return saveStateOnClose = false;
            }).fail(function (e) {
                return $modalmsg.show().html('<span class="text-danger">' + e.status + ': ' + e.statusText + '</span>');
            }).done(function (data) {
                if (data.success === true) {
                    $pagealrt.show().html('<div class="alert alert-info"><strong>Sent!</strong> Your message has been sent.</div>');
                    window.setTimeout(function () {
                        return $pagealrt.hide();
                    }, 3000);
                    $modal.modal('hide');
                } else {
        ...
        var splitRecipientString = function splitRecipientString(str) {
            return str.split(' ').filter(function (e) {
                return e.search(/^[A-Z0-9_]{3,20}$/i) === 0;
            });
        };

        var addRecipientLabel = function addRecipientLabel(recipient, style) {
            var id = recipient.toLowerCase();
            style = ['recipient', style];
            if (!$recipientscont.find('.recipient[data-recipient="' + id + '"]').get(0)) {
                $recipientscont.append('<span class="' + style.join(' ') + '" data-recipient="' + id + '">' + ('<span class="recipient-name">' + recipient + '</span>') + '<i class="glyphicon glyphicon-remove remove-recipient" title="Remove"></i>' + '</span>');
            }
        };

        var getRecipientLabels = function getRecipientLabels() {
            return $recipientscont.find('.recipient').get().map(function (e) {
                return $(e).data('recipient');
            });
        };

        $modal.on('keydown', function () {
            return $modalmsg.hide();
        });
        $modal.on('shown.bs.modal', function (e) {
            return $(e.currentTarget).find('input#compose-recipients').focus();
        });
        ...
        $modal.on('click', '.remove-recipient', function (e) {
            return $(e.currentTarget).closest('.recipient').remove();
        });
        ...
        $usergroups.on('click', '.groups a', function (e) {
            return addRecipientLabel($(e.currentTarget).text(), 'group');
        });

        $('#message-reply').on('click', function (e) {
            $modal.unbind('shown.bs.modal').on('shown.bs.modal', function (e) {
                return $(e.currentTarget).find('textarea').focus();
            });
            $modal.find('#composeLabel').text('Reply ...');
            $modal.find('.modal-recipients,.modal-settings,.modal-user-groups').hide();
            $recipients.val('');
            $recipientscont.empty();
...
$(function () {
    ...
    if (inboxtable.length > 0) {
        var displayInbox = function displayInbox(data) {
            start += 25;
            inboxloading.fadeOut();
            var container = inboxtable.find('tbody');
            var out = (data.length ? data : []).map(function (msg) {
                var message = msg.message.trim();
                formatters.forEach(function (f) {
                    return message = f.format(ctx, message);
                });
                return '' + ('<tr data-id="' + msg.userid + '" data-username="' + msg.user + '" class="' + (msg.unread <= 0 ? 'read' : 'unread') + '">') + '<td class="from">' + ('<a href="/profile/messages/' + msg.userid + '">' + msg.user + '</a> ') + ('<span class="count">(' + (msg.unread > 0 ? msg.unread : parseInt(msg.read) + parseInt(msg.unread)) + ')</span>') + '</td>' + ('<td class="message"><span>' + message + '</span></td>') + ('<td class="timestamp">' + msg.timestamp + '</td>') + '</tr>';
            });
        ...
        var loadInbox = function loadInbox() {
            inboxloading.fadeIn();
            return $.ajax({ url: '/api/messages/inbox', data: { s: start } }).done(displayInbox).fail(console.error);
        };

        var activateSelector = function activateSelector() {
            $(this).find('i').attr('class', 'fa fa-dot-circle-o');
            $(this).addClass('active');
        };

        var deactivateSelector = function deactivateSelector() {
            $(this).find('i').attr('class', 'fa fa-circle-o');
            $(this).removeClass('active');
        };

        var toggleRowSelector = function toggleRowSelector(e) {
            var self = $(this);
            e.preventDefault();
            e.stopPropagation();
        ...
        var toggleRowClick = function toggleRowClick(e) {
            var self = $(this),
                userid = self.data('id');
            if (userid !== undefined) {
                e.preventDefault();
        ...
        var pressedTableRow = function pressedTableRow() {
            $(this).addClass('pressed');
        };

        var releasedTableRow = function releasedTableRow() {
            $(this).removeClass('pressed');
        };

        inboxtable.each(function (i, el) {
            $(el).on('click', 'tbody tr', toggleRowClick).on('click', 'tbody td.selector', toggleRowSelector).on('mousedown', 'tbody tr', pressedTableRow).on('mouseup', 'tbody tr', releasedTableRow);
        });

        $('#mark-all').on('click', function () {
            $.ajax({ url: '/api/messages/open' }).always(function () {
        ...
        var start = 0;
        var messagemore = $('#inbox-show-more'),
...
$(function () {
    ...
    if (messagelist.length > 0) {
        var displayMessages = function displayMessages(data) {
            ...
            var out = (data.length ? data : []).reverse().map(function (msg) {
                var message = msg['message'].trim();
                formatters.forEach(function (f) {
                    return message = f.format(ctx, message);
                });
                var isme = parseInt(userid) !== parseInt(msg['userid']),
                    styles = [];
                styles.push('message-active');
                styles.push('message-' + (isme ? 'me' : 'notme'));
                styles.push(msg['isread'] === 1 ? 'message-read' : 'message-unread');
                return '' + ('<div id="' + msg['id'] + '" class="message ' + styles.join(' ') + ' content">') + '<div class="message-from">' + ('<div class="message-date pull-right"><i class="fa message-list-item-status pull-right ' + (isme ? 'read' : 'unread') + '"></i>  ' + msg['timestamp'] + '</div>') + ('<span title="' + msg['from'] + '">' + msg['from'] + '</span>') + '</div>' + '<div class="message-content">' + ('<div class="message-txt">' + message + '</div>') + '</div>' + '</div>';
            });
        ...
        var loadMessages = function loadMessages() {
            loading.fadeIn();
            return $.ajax({ url: '/api/messages/usr/' + encodeURIComponent(username.toLowerCase()) + '/inbox', data: { s: start } }).always(displayMessages);
        };

        var start = 0;

        showmore.on('click', function () {
            return loadMessages().done(function () {
