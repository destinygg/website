...
//
function swaptitle(title){0===a.length&&(a=[document.title]),a.push(title),iv||(iv=setInterval(function(){
// has document.title changed externally?
-1===a.indexOf(document.title)&&(
// update the default title
a[0]=document.title),document.title=a[++i%a.length]},1e3))}function swapTitleCancel(){
// dont do any more if we haven't got anything open
0!==a.length&&(
// if an IE overlay is present, kill it
"external"in window&&"msSiteModeClearIconOverlay"in window.external&&window.external.msSiteModeClearIconOverlay(),clearInterval(iv),iv=!1,document.title=a[0],a=[])}
//
// Add aevent handlers
function addEvent(el,name,func){if(name.match(" "))for(var a=name.split(" "),i=0;i<a.length;i++)addEvent(el,a[i],func);el.addEventListener?(el.removeEventListener(name,func,!1),el.addEventListener(name,func,!1)):(el.detachEvent("on"+name,func),el.attachEvent("on"+name,func))}function check_permission(){
...
// -1 (Notifications are not supported)
// IE9
return"external"in window&&"msIsSiteMode"in window.external?window.external.msIsSiteMode()?PERMISSION_GRANTED:PERMISSION_UNKNOWN:"webkitNotifications"in window?0===window.webkitNotifications.checkPermission()?PERMISSION_GRANTED:PERMISSION_DENIED:"mozNotification"in window.navigator?PERMISSION_GRANTED:PERMISSION_UNKNOWN}function update_permission(){
// Define the current state
return window.Notification.permission=check_permission(),window.Notification.permission}var iv,PERMISSION_GRANTED="granted",PERMISSION_DENIED="denied",PERMISSION_UNKNOWN="unknown",a=[],i=0;Object(window.Notification).permission||(
//
...
// Write
if(void 0!==value&&!$.isFunction(value)){if(options=$.extend({},config.defaults,options),"number"==typeof options.expires){var days=options.expires,t=options.expires=new Date;t.setTime(+t+864e5*days)}return document.cookie=[encode(key),"=",stringifyCookieValue(value),options.expires?"; expires="+options.expires.toUTCString():"",options.path?"; path="+options.path:"",options.domain?"; domain="+options.domain:"",options.secure?"; secure":""].join("")}for(var result=key?void 0:{},cookies=document.cookie?document.cookie.split("; "):[],i=0,l=cookies.length;l>i;i++){var parts=cookies[i].split("="),name=decode(parts.shift()),cookie=parts.join("=");if(key&&key===name){
...
// ======================
var dismiss='[data-dismiss="alert"]',Alert=function(el){$(el).on("click",dismiss,this.close)};Alert.VERSION="3.3.6",Alert.TRANSITION_DURATION=150,Alert.prototype.close=function(e){function removeElement(){
...
// ==============================
var Button=function(element,options){this.$element=$(element),this.options=$.extend({},Button.DEFAULTS,options),this.isLoading=!1};Button.VERSION="3.3.6",Button.DEFAULTS={loadingText:"loading..."},Button.prototype.setState=function(state){var d="disabled",$el=this.$element,val=$el.is("input")?"val":"html",data=$el.data();state+="Text",null==data.resetText&&$el.data("resetText",$el[val]()),setTimeout($.proxy(function(){$el[val](null==data[state]?this.options[state]:data[state]),"loadingText"==state?(this.isLoading=!0,$el.addClass(d).attr(d,d)):this.isLoading&&(this.isLoading=!1,$el.removeClass(d).removeAttr(d))},this),0)},Button.prototype.toggle=function(){var changed=!0,$parent=this.$element.closest('[data-toggle="buttons"]');if($parent.length){var $input=this.$element.find("input");"radio"==$input.prop("type")?($input.prop("checked")&&(changed=!1),$parent.find(".active").removeClass("active"),this.$element.addClass("active")):"checkbox"==$input.prop("type")&&($input.prop("checked")!==this.$element.hasClass("active")&&(changed=!1),this.$element.toggleClass("active")),$input.prop("checked",this.$element.hasClass("active")),changed&&$input.trigger("change")}else this.$element.attr("aria-pressed",!this.$element.hasClass("active")),this.$element.toggleClass("active")};var old=$.fn.button;$.fn.button=Plugin,$.fn.button.Constructor=Button,
// BUTTON NO CONFLICT
// ==================
...
// =========================
var backdrop=".dropdown-backdrop",toggle='[data-toggle="dropdown"]',Dropdown=function(element){$(element).on("click.bs.dropdown",this.toggle)};Dropdown.VERSION="3.3.6",Dropdown.prototype.toggle=function(e){var $this=$(this);if(!$this.is(".disabled, :disabled")){var $parent=getParent($this),isActive=$parent.hasClass("open");if(clearMenus(),!isActive){"ontouchstart"in document.documentElement&&!$parent.closest(".navbar-nav").length&&
