...
if(
//
options=options||{},this.body=options.body||"",this.icon=options.icon||"",this.lang=options.lang||"",this.tag=options.tag||"",this.close=function(){
// remove swapTitle
swapTitleCancel(),
// Close
Object(n).close&&n.close(),self.onclose()},this.onclick=function(){},this.onclose=function(){},
//
// Swap document.title
//
swaptitle(message),"external"in window&&"msIsSiteMode"in window.external)window.external.msIsSiteMode()&&(window.external.msSiteModeActivate(),this.icon&&window.external.msSiteModeSetIconOverlay(this.icon,message));else if("webkitNotifications"in window)0===window.webkitNotifications.checkPermission()&&(n=window.webkitNotifications.createNotification(this.icon,message,this.body),n.show(),n.onclick=function(){
// Fire any user bound events to the onclick function
self.onclick(),
// redirect the user back to the page
window.focus(),setTimeout(function(){n.cancel()},1e3)});else if("mozNotification"in window.navigator){var m=window.navigator.mozNotification.createNotification(message,this.body,this.icon);m.show()}},window.Notification.requestPermission=function(cb){
// IE9
if(
// Setup
// triggers the authentication to create a notification
cb=cb||function(){},"external"in window&&"msIsSiteMode"in window.external){try{window.external.msIsSiteMode()||(window.external.msAddSiteMode(),cb(PERMISSION_UNKNOWN))}catch(e){}cb(update_permission())}else"webkitNotifications"in window?window.webkitNotifications.requestPermission(function(){cb(update_permission())}):cb(update_permission())},
...
options.path?"; path="+options.path:"",options.domain?"; domain="+options.domain:"",options.secure?"; secure":""].join("")}for(var result=key?void 0:{},cookies=document.cookie?document.cookie.split("; "):[],i=0,l=cookies.length;i<l;i++){var parts=cookies[i].split("="),name=decode(parts.shift()),cookie=parts.join("=");if(key&&key===name){
...
// ==============================
var Button=function(element,options){this.$element=$(element),this.options=$.extend({},Button.DEFAULTS,options),this.isLoading=!1};Button.VERSION="3.3.6",Button.DEFAULTS={loadingText:"loading..."},Button.prototype.setState=function(state){var d="disabled",$el=this.$element,val=$el.is("input")?"val":"html",data=$el.data();state+="Text",null==data.resetText&&$el.data("resetText",$el[val]()),
