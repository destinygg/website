...
<?$summoners = Leagueapi::getInstance()->getSummoners ( array ('cacheFirst'=>true) );?>
<section id="lolpanel" class="container">
	<div class="content content-dark content-split clearfix">
	<?if(!empty($summoners)):?>
	<?foreach($summoners as $summoner):?>
		<div id="summoner-<?=$summoner->summonerId?>" class="summoner-stub clearfix">
			...
			<div class="summoner-rank-info unranked pull-right">
				<div class="pull-left summon-rank-display">
					...
					<div class="summoner-rank-thumbnail pull-left"><i title="Unknown" style="width:45px; height:45px; background: url(<?=Config::cdn()?>/img/lol/rank/unknown.png) no-repeat center center; background-size: 60px 60px;"></i></div>
				</div>
			</div>
			<?else:?>
			...
			<div class="summoner-rank-info ranked pull-right">
				...
				<div class="pull-left summon-rank-stats summon-mini-series">
					<div><span style="color: #b19e00;"><?=$summoner->league->miniSeries->target?></span> target game(s)</div>
					<div><span style="color: #1a6f00;"><?=$summoner->league->miniSeries->wins?></span> / <span style="color: #8a1919;"><?=$summoner->league->miniSeries->losses?></span> Win Loss</div>
				</div>
				<?endif;?>
				
				<div class="pull-left summon-rank-stats">
					<div><?=Tpl::out($summoner->league->leagueName)?></div>
					<div><span data-placement="left" rel="tooltip" title="Previous day position <?=$summoner->league->previousDayLeaguePosition?>">
						Position 
