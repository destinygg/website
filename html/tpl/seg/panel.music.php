...
<?
$tweets = Service\Twitter::getInstance()->getTimeline(array('cacheFirst'=>true))->getResponse();
$music = Service\Lastfm::getInstance()->getRecentTracks(array('cacheFirst'=>true))->getResponse();
?>
<section class="container">
	<div class="content content-dark clearfix">
		...
		<div id="stream-twitter" class="stream">
			<h3 class="title">
				<span>Tweets</span> <a href="https://twitter.com/<?=Config::$a['twitter']['user']?>/">twitter.com</a>
			</h3>
			<div class="entries">
			<?if(!empty($tweets)):?>
			<?foreach($tweets as $tweetIndex=>$tweet):?>
		...
		<div id="stream-lastfm" class="stream">
			<h3 class="title">
				<span>Music</span> <a href="http://www.last.fm/user/<?=Config::$a['lastfm']['user']?>">last.fm</a>
			</h3>
			<div class="entries">
			<?if(!empty($music) && isset($music->recenttracks->track) && !empty($music->recenttracks->track)):?>
			<?foreach($music->recenttracks->track as $trackIndex=>$track):?>
			<?if($trackIndex == 3){break;};?>
				<div class="media">
					<a class="pull-left cover-image" href="<?=$track->url?>">
						<img class="media-object" src="<?=Config::cdn()?>/img/64x64.gif" data-src="<?=$track->image[1]->{'#text'}?>">
					</a>
					<div class="media-body">
						<h4 class="media-heading trackname"><a href="<?=$track->url?>"><?=Tpl::out($track->name)?></a></h4>
